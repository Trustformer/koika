...
Lemma index_to_nat_injective {n: nat}:
  forall x y : index n, index_to_nat x = index_to_nat y -> x = y.
Proof.
...
Lemma index_to_nat_bounded {sz}: forall (idx: index sz), index_to_nat idx < sz.
Proof. induction sz; cbn; destruct idx; auto with arith. Qed.
...
Lemma index_to_nat_of_nat {sz}:
  ...
  index_of_nat sz n = Some idx -> index_to_nat idx = n.
Proof.
  induction sz; cbn.
  - destruct idx.
  - destruct n.
...
Lemma index_of_nat_to_nat {sz}:
  forall (idx: index sz), index_of_nat sz (index_to_nat idx) = Some idx.
Proof.
...
Lemma index_of_nat_none_ge : forall sz n, index_of_nat sz n = None -> n >= sz.
Proof.
...
Lemma index_of_nat_ge_none : forall sz n, n >= sz -> index_of_nat sz n = None.
Proof.
...
Definition index_of_nat_lt (sz n: nat) : n < sz -> index sz.
Proof.
...
Definition vect_tl {T n} (v: vect T (S n)) : vect T n := v.(vtl).
...
Proof. unfold vect_hd, vect_tl. reflexivity. Qed.
...
Fixpoint vect_app {T} {sz1 sz2} (v1: vect T sz1) (v2: vect T sz2) {struct sz1}
: vect T (sz1 + sz2) :=
...
Lemma vect_app_nil :
  ...
  vect_app v v0 = rew (vect_app_nil_cast _) in v.
Proof.
  destruct v0.
...
Lemma vect_app_cast_l {T} {sz1 sz1' sz2}:
  ...
  vect_app (rew h in v1) v2
  = rew [fun sz => vect T (sz + sz2)] h in (vect_app v1 v2).
Proof. destruct h; reflexivity. Defined.

Lemma vect_app_cast_r {T} {sz1 sz1' sz2}:
  forall (h: sz1 = sz1') (v1: vect T sz1) (v2: vect T sz2),
  vect_app v2 (rew h in v1)
  = rew [fun sz => vect T (sz2 + sz)] h in (vect_app v2 v1).
Proof. destruct h; reflexivity. Defined.
...
Fixpoint vect_split {T} {sz1 sz2} (v: vect T (sz1 + sz2)) {struct sz1}
: vect T sz1 * vect T sz2 :=
  match sz1 as n return (vect T (n + sz2) -> vect T n * vect T sz2) with
  | 0 => fun v => (vect_nil, v)
  | S sz1 => fun v =>
    let '(v1, v2) := vect_split (vect_tl v) in
...
Proof. destruct n; cbn; destruct idx; eauto. Defined.
...
Proof. induction sz; simpl; intros; try rewrite IHsz; reflexivity. Qed.
...
Fixpoint vect_nth_map {T T' sz} (f: T -> T') {struct sz}:
  forall (v: vect T sz) idx, vect_nth (vect_map f v) idx = f (vect_nth v idx).
Proof. destruct sz, idx; cbn; eauto. Defined.

Lemma vect_map_map {T T' T'' n} (f: T -> T') (f': T' -> T'') :
  forall (v: vect T n),
  vect_map f' (vect_map f v) = vect_map (fun x => f' (f x)) v.
Proof. induction n; cbn; intros; rewrite ?IHn; reflexivity. Qed.

Lemma vect_map_pointwise_morphism {T T' n} (f f': T -> T') :
  (forall x, f x = f' x) -> forall (v: vect T n), vect_map f v = vect_map f' v.
Proof.
  induction n; cbn; intros; rewrite ?H, ?IHn by eassumption; reflexivity.
Qed.

Lemma vect_map_id {T n} (f: T -> T):
  (forall x, f x = x) -> forall (v: vect T n), vect_map f v = v.
Proof.
  induction n; destruct v; cbn; intros; rewrite ?H, ?IHn by eassumption;
    reflexivity.
Qed.

Fixpoint vect_map2
  {T1 T2 T n} (f: T1 -> T2 -> T) (v1: vect T1 n) (v2: vect T2 n)
: vect T n :=
  match n return vect T1 n -> vect T2 n -> vect T n with
  | O => fun _ _ => vect_nil
  | S _ => fun v1 v2 =>
    vect_cons
      (f (vect_hd v1) (vect_hd v2)) (vect_map2 f (vect_tl v1) (vect_tl v2))
  end v1 v2.

Fixpoint vect_nth_map2
  {T1 T2 T n} (f: T1 -> T2 -> T) (v1: vect T1 n) (v2: vect T2 n) {struct n}
: forall idx,
  vect_nth (vect_map2 f v1 v2) idx = f (vect_nth v1 idx) (vect_nth v2 idx).
Proof. destruct n, idx; cbn; eauto. Defined.

Definition vect_zip {T1 T2 n} (v1: vect T1 n) (v2: vect T2 n)
: vect (T1 * T2) n :=
  vect_map2 (fun b1 b2 => (b1, b2)) v1 v2.

Definition vect_nth_zip {T1 T2 n} (v1: vect T1 n) (v2: vect T2 n) :
  forall idx,
  vect_nth (vect_zip v1 v2) idx = (vect_nth v1 idx, vect_nth v2 idx).
Proof. apply vect_nth_map2. Defined.
...
Fixpoint vect_skipn_plus_cast sz n: n + sz - n = sz.
...
Lemma vect_split_firstn_skipn :
  ...
  vect_split v = (vect_firstn_plus sz v, vect_skipn_plus sz v).
Proof.
...
Fixpoint vect_extend_beginning_cast' x y: x + S y = S (x + y).
