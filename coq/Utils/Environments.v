...
Require Export Koika.Utils.Member.
...
Definition ContextEnv {K} {FT: FiniteType K}: Env K.
  unshelve refine {|
    env_t V := context V finite_elements;
    getenv _ ctx k := cassoc (finite_member k) ctx;
    putenv _ ctx k v := creplace (finite_member k) v ctx;
    create _ fn := ccreate finite_elements (fun k _ => fn k)
  |}.
