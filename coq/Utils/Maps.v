...
Proof.
  ...
  tauto.
  inversion H; subst. rewrite IHl1 in H3. rewrite in_app_iff in H2.
  intuition.
  constructor; auto. red; intros. elim H2; intro. congruence. auto.
  destruct H as [B [C D]]. inversion B; subst.
  constructor. rewrite in_app_iff. intuition. elim (D a a); auto. apply in_eq.
...
Module PTree <: TREE.
  ...
  Theorem gsspec:
    forall (A: Type) (i j: positive) (x: A) (m: t A),
    get i (set j x m) = if Pos.eq_dec i j then Some x else get i m.
  Proof.
    intros.
    destruct (Pos.eq_dec i j); [ rewrite e; apply gss | apply gso; auto ].
  Qed.
  ...
  Variable f: elt -> option A -> option B -> option C.
  Hypothesis f_none_none: forall i, f i None None = None.

  Fixpoint xcombine_l i (m : t A) {struct m} : t C :=
      match m with
      | Leaf => Leaf
      | Node l o r => Node' (xcombine_l (xO i) l) (f (prev i) o None) (xcombine_l (xI i) r)
      end.
  ...
  Fixpoint combine' i (m1: t A) (m2: t B) {struct m1} : t C :=
    match m1 with
    | Leaf => xcombine_r i m2
    | Node l1 o1 r1 =>
        match m2 with
        | Leaf => xcombine_l i m1
        | Node l2 o2 r2 => Node' (combine' (xO i) l1 l2) (f (prev i) o1 o2) (combine' (xI i) r1 r2)
        end
    end.
  ...
  Proof.
    ...
    rewrite prev_append_prev. simpl. auto.
  Qed.
  ...
  Proof.
    intros A B f g EQ1.
    assert (EQ2: forall (i j: option A) p, g p i j = f p j i).
      intros; auto.
  ...
  - rewrite xelements_node in H. rewrite ! in_app_iff in H. destruct H as [P | [P | P]].
    + edestruct IHm1 as (j & Q & R). eauto. exists (xO j); auto.
    + destruct o; simpl in P; intuition auto. inv H. exists xH; auto.
    + edestruct IHm2 as (j & Q & R). eauto. exists (xI j); auto.
  Qed.
  ...
  Theorem elements_complete:
    ...
    In (i, v) (elements m) -> get i m = Some v.
  Proof.
    unfold elements. intros A m i v H. edestruct in_xelements as (j & P & Q); eauto.
    rewrite prev_append_prev in P. change i with (prev_append 1 i) in P.
    apply prev_append_inj in P; eauto. intros; congruence.
  Qed.
  ...
  Proof.
    ...
    edestruct in_xelements as (kk & P & Q); eauto.
  Qed.
  ...
  - assert (NOTIN1: ~ In (prev i) (xkeys m1 (xO i))).
    ...
    rewrite xkeys_node. apply NoDup_app. split; [|split]. auto.
  ...
  + rewrite (xelements_empty (Node m1 o m2)). simpl; constructor.
    intros. specialize (H i). rewrite gempty in H. inv H; auto.
  + rewrite ! xelements_node. repeat apply Forall2_app.
  ...
  Proof.
    ...
    induction 1. auto. destruct x, y; simpl in *. destruct H0; subst. congruence.
  Qed.
...
Module PMap <: MAP.
  ...
  Theorem gsspec:
    forall (A: Type) (i j: positive) (x: A) (m: t A),
    get i (set j x m) = if Pos.eq_dec i j then x else get i m.
  Proof.
    intros. destruct (Pos.eq_dec i j).
  ...
  Proof.
    intros. unfold set. simpl. rewrite PTree.set2. auto.
  Qed.
...
Module IMap(X: INDEXED_TYPE).
  ...
  Proof.
    ...
    rewrite PMap.gsspec.
    case (X.eq i j); intro.
    subst j. destruct Pos.eq_dec; congruence.
    destruct Pos.eq_dec. apply X.index_inj in e. congruence. reflexivity.
  Qed.
