...
Section SyntaxMacros.
  ...
  Definition ustruct_init (sig: struct_sig) (fields: list (string * uaction))
  : uaction :=
  ...
  Fixpoint uswitch
    (var: uaction) (default: uaction) (branches: list (uaction * uaction))
  : uaction :=
    match branches with
    | nil => default
  ...
  Fixpoint uswitch_nodefault
    (var: uaction) {nb} (branches: vect (uaction * uaction) (S nb))
  : uaction :=
    ...
    | S nb => fun branches =>
      UIf
        (UBinop (UEq false) var label) action
  ...
  : vect (uaction * uaction) bound :=
    let label_of_index idx :=
      UConst (tau := bits_t _) (Bits.of_index label_sz idx) in
    vect_map
      (fun idx => (label_of_index idx, branch_bodies idx))
      (all_indices bound).

  Fixpoint uswitch_stateful
    (var: uaction) {nb} (branches: vect (uaction * uaction) nb)
  : uaction :=
    ...
    | S nb => fun branches =>
      let '(label, action) := vect_hd branches in
      USeq
        (UIf (UBinop (UEq false) var label) action uskip)
        (uswitch_stateful var (vect_tl branches))
    end branches.

  Fixpoint muxtree
    var_logsz bit_idx (var: var_t) {sz} (bodies: bits sz -> uaction)
  :=
    ...
    | S n =>
      ...
      let bidx :=
        UConst (tau := bits_t var_logsz) (Bits.of_nat var_logsz bit_idx) in
      UIf
        (UBinop (UBits2 USel) (UVar var) bidx)
        (muxtree var_logsz (S bit_idx) var (fun bs => bodies bs~1))
        (muxtree var_logsz (S bit_idx) var (fun bs => bodies bs~0))
    end bodies.

  Definition UCompleteMuxTree
    sz (var: var_t) (branch_bodies: bits sz -> uaction)
  :=
  ...
  Definition UCompleteOrTree
    sz nbits (var: var_t) (branch_bodies: bits sz -> uaction)
  :=
    ortree
      (fun bs =>
        UIf
          (UBinop (UEq false) (UConst (tau := bits_t sz) bs) (UVar var))
          (branch_bodies bs)
  ...
  Definition UCompleteSwitch
    (style: switch_style) sz (var: var_t)
    (branch_bodies: index (pow2 sz) -> uaction)
  :=
    ...
    | OrTreeSwitch nbits =>
      UCompleteOrTree
        sz nbits var (fun bs => branch_bodies (Bits.to_index_safe bs))
    | NestedSwitch => uswitch_nodefault (UVar var) (branches branch_bodies)
    | SequentialSwitchTt =>
...
Module Display.
  Section Display.
    ...
    Fixpoint make_printer (offset: nat) (fstring: list field) : intfun :=
      ...
      | [] => empty_printer
      | f :: fstring =>
        extend_printer f offset (make_printer (S offset) fstring)
      end.

    Definition Printer (fstring: list field) := make_printer 0 fstring.

    Example example :=
      Eval compute in
        Printer [Str "x: "; Value (bits_t 16); Str "y: "; Value (bits_t 32)].
  End Display.
End Display.
...
Section LoweredSyntaxMacros.
  ...
  Fixpoint infix_action
    (infix: lsig) {sig sig': lsig} {tau} (a: action (sig ++ sig') tau)
  ...
  Proof.
    ...
    - exact
      (Bind
        k (infix_action infix _ _ _ a1)
        (infix_action infix (_ :: sig) sig' _ a2)).
    - exact
      (If
        (infix_action infix _ _ _ a1) (infix_action infix _ _ _ a2)
        (infix_action infix _ _ _ a3)).
    - exact (Read port idx).
    - exact (Write port idx (infix_action infix _ _ _ a)).
    - exact (Unop fn (infix_action infix _ _ _ a)).
    - exact
      (Binop fn (infix_action infix _ _ _ a1) (infix_action infix _ _ _ a2)).
  ...
  Proof.
    rewrite <- (capp_nil_r suffix); apply infix_action;
      rewrite (capp_nil_r sig); exact a.
  Defined.
  ...
  Fixpoint InternalCall'
    {sz: nat} {sig: lsig} {fn_sig: lsig}
    (args: context (fun sz => var_t * action sig sz)%type fn_sig)
    (fn_body: action (fn_sig ++ sig) sz)
  : action sig sz :=
    match args in context _ fn_sig
      return action (fn_sig ++ sig) sz -> action sig sz
    with
    | CtxEmpty => fun fn_body => fn_body
    | CtxCons sz (k, v) tl => fun fn_body =>
