...
Require Import Koika.Common Koika.Environments Koika.Syntax Koika.TypedSyntax
  Koika.ErrorReporting Koika.SyntaxMacros.
...
Section Desugaring.
  ...
  Fixpoint desugar_action'
    {reg_t' ext_fn_t'} (pos: pos_t) (fR: reg_t' -> reg_t)
    (fSigma: ext_fn_t' -> ext_fn_t) (a: uaction reg_t' ext_fn_t') {struct a}
  ...
  : uaction reg_t ext_fn_t :=
    ...
    | UArrayInit tau elements =>
      ...
      snd
        (List.fold_left
          (fun '(pos, acc) a => (S pos, (usubst pos) acc (d a)))
          elements (0, empty))
    | USwitch var default branches =>
      let branches :=
        List.map (fun '(cond, body) => (d cond, d body)) branches in
      SyntaxMacros.uswitch (d var) (d default) branches
    | UCallModule fR' fSigma' fn args =>
      let df body :=
        desugar_action'
          pos (fun r => fR (fR' r)) (fun fn => fSigma (fSigma' fn)) body in
