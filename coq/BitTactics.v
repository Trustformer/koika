...
Definition bits_get_size {sz} (bs: bits sz) := sz.
...
Ltac Bits_to_N_t :=
  ...
  repeat
    ...
    | [ H: Bits.single (Bits.neg _) = true |- _ ] => apply negb_true_iff in H
    | [ H: Bits.single (Bits.neg _) = false |- _ ] => apply negb_false_iff in H
    ...
    | [ H: Bits.single ?bs = _ |- _ ] => apply Bits.single_to_bits in H
    | _ => progress (rewrite sel_spec in * )
    | [ H: context[Bits.to_N (BitFuns.lsl ?x ?y)] |- _ ] =>
      rewrite to_N_lsl in H
    | [ |- context[Bits.to_N (BitFuns.lsl ?x ?y)] ] => rewrite to_N_lsl
    ...
    | [ H: _ = _ |- _ ] => apply (f_equal Bits.to_N) in H
    | [ H: _ <> _ |- _ ] => apply Bits.to_N_inj_contra in H
    end.

Ltac remember_bits_to_N :=
  repeat
    match goal with
    | [ |- context[Bits.to_N ?bs] ] => remember_once (Bits.to_N bs)
    | [ H: context[Bits.to_N ?bs] |- _ ] => remember_once (Bits.to_N bs)
    end.
...
Ltac lia_bits :=
  Bits_to_N_t; cbn in *; pose_bits_bound_proofs; remember_bits_to_N; cbn in *;
    lia.
