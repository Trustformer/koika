...
Section Normalize.
  ...
  : uaction pos_t var_t fn_name_t reg_t ext_fn_t := (*
    ...
    inlined. We do not have to care about reads at all as whenever at least a
    read occurs in a rule, subsequent reads are not a problem.
    However, writes are removed - this is not a problem as far as the final
    value of the argument is concerned as writes are of type unit_t anyways. *)
    match ua with
  ...
  Fixpoint remove_first_n_internal_calls_aux
    (ua: uaction pos_t var_t fn_name_t reg_t ext_fn_t) (lz: list zipper)
    (n: nat)
  : option (uaction pos_t var_t fn_name_t reg_t ext_fn_t) :=
    match n with
    | O => Some ua
