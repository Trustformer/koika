...
Section SimpleFormInterpretation.
  ...
  Proof.
    ...
    - assert ( wt_sact (Sigma:=Sigma) R vss
      ...
      destr; auto.
      + clear H2. inv H1.
        destr. exploit eval_sact_no_vars_wt. apply IHwt_sact1. eauto.
        intro WT1. inv WT1.
        destruct (eval_sact_no_vars (simplify_sact a2)) eqn:?.
        exploit eval_sact_no_vars_wt. apply IHwt_sact2. eauto.
        intro WT2. inv WT2.
  ...
  Proof.
    ...
    - destr_in EV_INIT.
      ...
      apply wt_simplify_sact. eauto. intro WT. apply wt_val_bool in WT.
      destruct WT; subst.
    ...
    - destruct (eval_sact_no_vars (simplify_sact a1)) eqn:?.
      exploit eval_sact_no_vars_interp. apply Heqo. intro IS1.
      destruct (eval_sact_no_vars (simplify_sact a2)) eqn:?.
      exploit eval_sact_no_vars_interp. apply Heqo0. intro IS2.
      + destr_in EV_INIT.
  ...
  Lemma var_in_sact_reachable:
    forall vvs a v, var_in_sact a v -> reachable_var vvs a v.
  ...
  Proof.
    ...
    - generalize (wf_sf_replace_field _ _ _ _ GF WF). intro A; inv WF; inv A.
      ...
      + inv A. rewrite H2 in H1; inv H1.
        ...
        eapply wf_sf_vvs0. eauto. auto. eauto. eauto.
      + inv A.
        ...
        eapply reachable_var_aux_below; eauto.
        eapply wf_sf_vvs0. eauto. apply H3. eauto. eauto.
    - intros. simpl.
      split. eapply f_wt_sact_ok'; eauto.
      intro A; inv A.
      rewrite PTree.gmap in H1.
  ...
  Lemma replace_field_interp_cycle_ok:
    ...
    get_field (getenv REnv r str) field = Some field_v
    -> getenv REnv (interp_cycle sf) reg
    = getenv REnv (interp_cycle (replace_field str sf field field_v)) reg.
  Proof.
    intros.
    eapply sf_eq_interp_cycle_ok.
    - auto.
