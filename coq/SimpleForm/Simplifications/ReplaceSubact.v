...
Section ReplaceSubact.
  ...
  Lemma interp_sact_replace_subact':
    forall
      vvs (WTvvs: wt_vvs R vvs) (VSV: vvs_smaller_variables vvs) idx v
      (SAME1: forall ov,
      ...
      (tt: type) (OLDTYPE: wt_sact R vvs (SVar idx) tt)
      (NEWTYPE: wt_sact R vvs v tt) a t (WT: wt_sact R vvs a t),
    forall vv,
    interp_sact (sigma:=sigma) REnv r vvs a vv
    <-> interp_sact
          (sigma:=sigma) REnv r (replace_subact_in_vars vvs idx v) a vv.
  Proof.
    intros.
  ...
  Lemma wf_sf_replace_subact':
    forall
      reg val sf tt (OLDTYPE: wt_sact R (vars sf) (SVar reg) tt)
  ...
  Lemma interp_sact_change_vvs:
    forall
      vvs1 (VSV1: vvs_smaller_variables vvs1)
      vvs2 (VSV2: vvs_smaller_variables vvs2) a v
      (IS: interp_sact (sigma:=sigma) REnv r vvs1 a v) t
      (WT1: wt_sact R vvs1 a t) (WT2: wt_sact R vvs2 a t)
      (SMALLER: forall var,
        reachable_var vvs1 a var -> vvs1 ! var = vvs2 ! var),
  ...
  Lemma replace_subact_interp_cycle_ok:
    forall
      {reg} {idx} {sf} {v} (WFSF: wf_sf sf) (VS: var_simpl (vars sf) idx v),
    getenv REnv (interp_cycle r sigma sf) reg
    = getenv REnv (interp_cycle r sigma (replace_subact sf idx v)) reg.
  Proof.
    intros. destruct VS.
