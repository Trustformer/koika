...
Require Import Koika.KoikaForm.ErrorReporting.

Section ErrorReporting.
  Context {pos_t var_t fn_name_t reg_t ext_fn_t: Type}.
  Context {R: reg_t -> type} {Sigma: ext_fn_t -> ExternalSignature}.

  Definition lift_basic_error_message
    (pos: pos_t) {sig tau} (e: action pos_t var_t fn_name_t R Sigma sig tau)
    (err: basic_error_message)
  : error pos_t var_t fn_name_t := {|
    epos := pos;
    emsg := BasicError err;
    esource := ErrSrc e |}.

  Definition lift_fn1_tc_result
    {A sig tau} pos (e: action pos_t var_t fn_name_t R Sigma sig tau)
    (r: result A fn_tc_error)
  : result A (error pos_t var_t fn_name_t) :=
    result_map_failure
      (fun '(side, err) => lift_basic_error_message pos e err) r.

  Definition lift_fn2_tc_result
    {A sig1 tau1 sig2 tau2} pos1
    (e1: action pos_t var_t fn_name_t R Sigma sig1 tau1)
...
Section TypeInference.
  ...
  Section Action.
    ...
    Definition cast_action'
      (pos: pos_t) {sig tau1} tau2 (e: action sig tau1) (emsg: error_message)
    ...
    Fixpoint assert_argtypes'
      {T} {sig} (src: T) nexpected (fn_name: fn_name_t) pos
      (args_desc: tsig var_t)
      (args: list (pos_t * {tau : type & action sig tau}))
    : result (
      context
        (K := (var_t * type)) (fun k_tau => action sig (snd k_tau)) args_desc)
    :=
      ...
      | [], _ =>
        Failure (
          mkerror
            pos (TooManyArguments fn_name nexpected (List.length args)) src)
      | _, [] =>
        Failure (
          mkerror
            pos (TooFewArguments fn_name nexpected (List.length args_desc)) src)
    ...
    Definition assert_argtypes
      {T} {sig} (src: T) (fn_name: fn_name_t) pos (args_desc: tsig var_t)
      (args: list (pos_t * {tau : type & action sig tau}))
    : result (
        context
          (K := (var_t * type)) (fun k_tau => action sig (snd k_tau)) args_desc)
    ...
    Fixpoint type_action
      (pos: pos_t) (sig: tsig var_t) (e: uaction reg_t ext_fn_t) {struct e}
    : result ({ tau: type & action sig tau }) :=
      ...
      | UVar var =>
        let/res tau_m :=
          opt_result (assoc var sig) (mkerror pos (UnboundVariable var) e) in
      ...
      | UIf cond tbranch fbranch =>
        ...
        let/res fbranch' :=
          cast_action (actpos pos fbranch) (`tbranch') (``fbranch') in
      ...
      | UInternalCall fn args =>
        ...
        let/res args_ctx :=
          assert_argtypes
            e fn.(int_name) pos (List.rev fn.(int_argspec))
            (List.rev tc_args_w_pos) in
        let/res fn_body' :=
          type_action
            (actpos pos fn.(int_body)) (List.rev fn.(int_argspec))
            fn.(int_body) in
        let/res fn_body' :=
          cast_action (actpos pos fn.(int_body)) fn.(int_retSig) (``fn_body') in
      ...
      | UUnop fn arg1 =>
        ...
        let/res fn :=
          lift_fn1_tc_result pos1 ``arg1' (PrimTypeInference.tc1 fn `arg1') in
        let/res arg1' :=
          cast_action pos1 (arg1Sig (PrimSignatures.Sigma1 fn)) (``arg1') in
      ...
      | UBinop fn arg1 arg2 =>
        ...
        let/res fn :=
          lift_fn2_tc_result pos1 ``arg1' pos2 ``arg2'
          (PrimTypeInference.tc2 fn `arg1' `arg2')
        in
        let/res arg1' :=
          cast_action pos1 (arg1Sig (PrimSignatures.Sigma2 fn)) (``arg1')
        in
        let/res arg2' :=
          cast_action pos2 (arg2Sig (PrimSignatures.Sigma2 fn)) (``arg2')
        in
        Success (EX (Binop fn arg1' arg2'))
      | UExternalCall fn arg1 =>
        let pos1 := actpos pos arg1 in
        let/res arg1' := type_action pos1 sig arg1 in
        let/res arg1' := cast_action pos1 (arg1Sig (Sigma fn)) (``arg1') in
    ...
    Definition tc_action
      (pos: pos_t) (sig: tsig var_t) (expected_tau: type)
      (e: uaction reg_t ext_fn_t)
    : result (action sig expected_tau) :=
