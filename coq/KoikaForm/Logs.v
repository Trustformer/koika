...
Require Export Koika.KoikaForm.Typed.TypedSyntax.

Section Logs.
  Inductive LogEntryKind := LogRead | LogWrite.

  Record LogEntry {T} := LE {
    kind: LogEntryKind;
    port: Port;
    val:
      match kind with
  ...
  Definition RLog T := list (@LogEntry T).
  ...
  Definition log_empty : Log := REnv.(create) (fun _ => []).

  Definition log_app (l1 l2: Log) :=
    map2 REnv (fun _ ll1 ll2 => ll1 ++ ll2) l1 l2.

  Definition log_find {T} (log: Log) reg (f: @LogEntry (R reg) -> option T)
  : option T :=
...
Section Maps.
  ...
  Definition RLog_map {T T'} (f: T -> T') l := List.map (LogEntry_map f) l.

  Definition log_map (f: forall idx, RLog (R1 idx) -> RLog (R2 idx)) (log: Log1)
  : Log2 :=
    Environments.map REnv (fun k l1 => f k l1) log.

  Definition log_map_values (f: forall idx, R1 idx -> R2 idx) (log: Log1)
  : Log2 :=
    log_map (fun k => RLog_map (f k)) log.
End Maps.

Definition Log {reg_t} R REnv :=
  @_Log reg_t (fun idx => type_denote (R idx)) REnv.
Definition CLog {reg_t} R REnv :=
  @_Log reg_t (fun idx => Bits.bits (R idx)) REnv.
