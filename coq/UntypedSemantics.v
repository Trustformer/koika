...
Section Interp.
  ...
  Section Action.
    ...
    Lemma cassoc_ucassoc:
      forall (sig: tsig var_t) (ND: NoDup (map fst sig)) (Gamma: tcontext sig)
        k,
    ...
    | wt_bits: forall n bs,
      List.length bs = n -> wt_val (bits_t n) (Bits n bs)
    ...
    :=
      let uvalue_of_struct_bits := (
        fix uvalue_of_struct_bits
          {fields: list (string * type)} (bs: list bool)
        : option (list val) :=
          match fields with
          | [] => Some []
          | (nm, tau)::fields =>
    ...
    Proof.
      ...
      - revert sig lv H H0. destruct sig. simpl.
        ...
        + inv H. inv H0. destruct a. simpl in *.
          ...
          match type of A with context[ let/opt _ := ?a in _ ]
          => destruct a eqn:?
          end; simpl in *; try congruence.
          rewrite H4. simpl. inv A; auto. auto.
      - assert (Forall (
          fun v => wt_val (array_type sig) v
    ...
    Proof.
      ...
      - destruct fn.
        + destr_in H; try congruence. inv H. simpl in *. inv Heqr.
          subst. reflexivity.
        + subst. inv H.
      ...
      - destr_in H.
        ...
        + destr_in H; inv H.
          simpl PrimSpecs.sigma1.
          simpl.
          erewrite find_field_offset_right_spec; eauto. simpl. destr. destr.
          simpl. f_equal.
    ...
    Definition list_eq_dec'
      {A: Type} (l1 l2: list A)
      (Aeq: forall x y, In x l1 -> {x = y} + {x <> y})
      : {l1 = l2} + {l1 <> l2}.
    Proof.
    ...
    Proof.
      ...
      - destruct v2; try (right; intro A; inv A; congruence).
        ...
        destruct (eq_dec bs v);
          try (right; intro A; inv A; congruence).
        subst; left; reflexivity.
      - destruct v2; try (right; intro A; inv A; congruence).
        destruct (struct_sig_eq_dec sig sig0);
          try (right; intro A; inv A; congruence).
        destruct (list_eq_dec' lv v);
          try (right; intro A; inv A; congruence). eauto.
        left; subst. reflexivity.
      - destruct v2; try (right; intro A; inv A; congruence).
    ...
    Lemma and_correct:
      forall sz
        (arg1:
          arg1Sig (PrimSignatures.Sigma2 (PrimTyped.Bits2 (PrimTyped.And sz)))
        )
        (arg2:
          arg2Sig (PrimSignatures.Sigma2 (PrimTyped.Bits2 (PrimTyped.And sz)))
        ) ret,
    ...
    Definition ubits2_sigma (ub: ubits2) (v1 v2: list bool) : list bool :=
      ...
      | ULsr =>
        let amount := Bits.to_nat (vect_of_list v2) in
        Nat.iter amount (fun v =>
          if eq_dec (List.length v1) O then [] else tl v ++ [false]
        ) v1
      | UAsr =>
        let amount := Bits.to_nat (vect_of_list v2) in
        Nat.iter amount (fun v =>
          if eq_dec (List.length v1) O then [] else tl v ++ [last v false]
        ) v1
      | UConcat => v2 ++ v1
      ...
      | UPlus =>
        vect_to_list (
          Bits.of_N (List.length v1) (Bits.to_N (vect_of_list v1)
          + Bits.to_N (vect_of_list v2))
        )
      | UMinus =>
        vect_to_list (
          ...
          (Bits.to_N (
            Bits.of_N (List.length v1) (
              Bits.to_N (vect_of_list v1)
              + Bits.to_N (Bits.neg (vect_of_list v2))
            )
          ) + Bits.to_N (sz:=List.length v1) Bits.one)
        )
      | UMul =>
        vect_to_list (
          Bits.of_N (List.length v1 + List.length v2)
    ...
    Lemma asr1:
      forall sz (v: bits sz),
      sz <> 0 ->
        vect_to_list (Bits.asr1 v)
        = tl (vect_to_list v) ++ [last (vect_to_list v) false].
    Proof.
    ...
    Lemma iter_list_vect:
      forall sz (v: bits sz) (f: list bool -> list bool)
        (g: bits sz -> bits sz),
    ...
    Lemma bits_map_rew:
      ...
      Bits.map f (rew [Bits.bits] pf in v)
        = rew [Bits.bits] pf in (Bits.map f v).
    Proof.
    ...
    Lemma lift_comparison_rew:
      forall {A} sz1 sz1' (pf: sz1 = sz1') (pf2: sz1 = sz1') (v1: bits sz1)
    ...
    Proof.
      ...
      - rewrite sel. rewrite vect_of_list_to_list.
        unfold Bits.to_nat; rewrite Bits.to_N_rew. auto.
      - rewrite vect_to_list_length.
        rewrite slice_subst. reflexivity.
      - rewrite slice.
        rewrite vect_of_list_to_list. unfold Bits.to_nat; rewrite Bits.to_N_rew.
        destr. destr. f_equal. f_equal. rewrite vect_to_list_length.
        reflexivity.
      - unfold Bits.plus.
        rewrite ! vect_of_list_to_list.
        rewrite ! Bits.to_N_rew.
        eapply vect_to_list_eq.
        erewrite (f_equal_dep _ (fun x => Bits.of_N x (
          Bits.to_N arg1 + Bits.to_N arg2
        ))).
      ...
      - destr.
        + rewrite ! vect_of_list_to_list.
          rewrite rew_compose.
          transitivity (vect_to_list (BitFuns.bitfun_of_predicate (
            if signed then
              match c with
              | cLt =>  Bits.signed_lt
              | cGt =>  Bits.signed_gt
    ...
    :=
      ...
      | p :: fields0 =>
        let (_, tau)
          as p0 return (snd p0 * struct_denote fields0 -> list val)
          := p in
...
Section Eq.
  ...
  Definition env_t_R
    {K: Type} {V1 V2: K -> Type} (R: forall k, V1 k -> V2 k -> Prop)
    (e: Env K) (e1: env_t e (fun x => V1 x)) (e2: env_t e (fun x => V2 x))
  : Prop :=
  ...
  Lemma interp_args_correct:
    let PP := fun u : uaction =>
      ...
      Success
        (existT (fun tau0 : type =>
            TypedSyntax.action pos_t var_t fn_name_t TR Sigma sig tau0
        ) tau a) ->
      forall
    ...
    log_eq REnv uaction_log action_log ->
    forall
      s0 : context (fun k_tau : var_t * type =>
        TypedSyntax.action pos_t var_t fn_name_t TR Sigma sig (snd k_tau)
      ) l,
    assert_argtypes' l (combine (map (TypeInference.actpos p) args') s')
    = Success s0 -> Datatypes.length args' = Datatypes.length s' ->
    forall
    ...
    TypedSemantics.interp_args'
      ...
      ) Gamma sched_log action_log s0 = Some (action_log', v, Gamma')
    -> exists
      (uaction_log' : Log) (res : list val) (UGamma' : list (var_t * val)),
    fold_right (
      ...
      =>
        let/opt3 action_log0, l0, Gamma0 := acc in (
          let/opt3 action_log1, v0, Gamma1 :=
           ...
           in Some (action_log1, v0 :: l0, Gamma1)
        )
    ...
    - destr_in TA; [|inv TA].
      ...
      assert (
        ...
        = Success s ->
          ...
          log_eq REnv usched_log sched_log ->
          log_eq REnv uaction_log action_log ->
          forall (s0 : context (fun k_tau : var_t * type =>
            TypedSyntax.action pos_t var_t fn_name_t TR Sigma sig (snd k_tau)
          ) (rev (int_argspec ufn))),
          assert_argtypes'
            (rev (int_argspec ufn))
          ...
          fold_right (
            fun a0
              (acc : option (Log * list val * list (var_t * val))) =>
                let/opt3 action_log0, l0, Gamma0 := acc in (
                  let/opt3 action_log1, v0, Gamma1 :=
                    interp_action r sigma Gamma0 usched_log action_log0 a0
                  in
  ...
  Proof.
    ...
    - destr_in TA; [|inv TA].
      ...
      assert (
        ...
        = Success s0 ->
        List.length args = List.length (rev s) ->
        TypedSemantics.interp_args' (@TypedSemantics.interp_action
          pos_t var_t fn_name_t reg_t ext_fn_t TR Sigma REnv tr tsigma
        ) Gamma sched_log action_log s0 = None ->
        fold_right (
        ...
        ) (Some (uaction_log, [], UGamma)) (rev args) = None
      ).
      {
  ...
  Lemma interp_rule_correct:
    forall
      ...
      sched_log usched_log (SL: log_eq REnv usched_log sched_log) action_log,
    ...
    = Some (uaction_log) /\ log_eq REnv uaction_log action_log.
  Proof.
    intros.
  ...
  Lemma interp_scheduler_correct:
    ...
    forall s l2, TypedSemantics.interp_scheduler tr tsigma rules s = l2 ->
    exists ul2, interp_scheduler r sigma urules s = ul2 /\ log_eq REnv ul2 l2.
  Proof.
...
Section Final.
  ...
  Lemma latest_write_eq:
    forall ulog log, log_eq (TR:=TR) REnv ulog log -> forall idx,
