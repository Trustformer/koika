...
Section Interp.
  ...
  | interp_list_cons:
      forall (Gamma: list (var_t * val))
        ...
        interp_list r sigma i Gamma' sched_log action_log' ar action_log'' vr Gamma'' ->
        interp_list r sigma i Gamma sched_log action_log (a::ar) action_log'' (vr++[v]) Gamma''
  .

  Lemma fold_left_rec:
    forall {reg_t ext_fn_t: Type}
           {REnv: Env reg_t}
           (f: forall     (Gamma: list (var_t * val))
                     (sched_log: Log REnv)
                     (action_log: Log REnv)
                     (a: Syntax.uaction pos_t var_t fn_name_t reg_t ext_fn_t),
      ...
      fold_left (fun acc a =>
                   ...
                   let/opt3 action_log, v, Gamma :=
                      f Gamma sched_log action_log a in
                   Some (action_log, v::l, Gamma))
                l
                (Some (action_log, X, Gamma)) =
      match     fold_left (fun acc a =>
                             let/opt3 action_log, l, Gamma := acc in
                             let/opt3 action_log, v, Gamma :=
                                f Gamma sched_log action_log a in
                             Some (action_log, v::l, Gamma))
                          l
                          (Some (action_log, [], Gamma)) with
        None => None
      | Some (al, l, Gamma) => Some (al, l ++ X, Gamma)
      end.
  Proof.
    induction l; simpl; intros; eauto.
    destruct (f Gamma sched_log action_log a) eqn:?; simpl; auto. destruct p. destruct p.
    rewrite (IHl (v::X)).
    rewrite (IHl [v]).
  ...
  Lemma interp_list_ok:
    ...
    forall Gamma action_log sched_log action_log' Gamma' lv,
      fold_left (fun acc a =>
                        ...
                        Some (action_log, v::l, Gamma))
                     l
                     (Some (action_log, [], Gamma)) = Some (action_log', lv, Gamma') <->
      interp_list r sigma i Gamma sched_log action_log l action_log' lv Gamma'.
  Proof.
    induction l; simpl; intros; eauto.
  ...
  | interp_list_ctx_cons:
      forall (Gamma: list (var_t * val))
        ...
        i Gamma sched_log action_log a action_log' v Gamma' ->
        interp_list_ctx r sigma i ((var,v)::Gamma') sched_log action_log' ar action_log'' Gamma'' ->
        interp_list_ctx r sigma i Gamma sched_log action_log ((var,a)::ar) action_log'' Gamma''
  .
   ...
   Lemma interp_list_ctx_ok:
    ...
    forall Gamma action_log sched_log action_log' Gamma',
      List.fold_left (fun acc '(var, a) =>
                          ...
                          ) l (Some (action_log, Gamma)) = Some (action_log', Gamma') <->
      interp_list_ctx r sigma i Gamma sched_log action_log l action_log' Gamma'.
  Proof.
    induction l; simpl; intros; eauto.
  ...
  Lemma structinit_ind_ok:
      ...
      fields (iok:
       ...
       Gamma Gamma' action_log action_log' vs v sig,
        (exists vfields,
            interp_list r sigma i
                     Gamma sched_log action_log (map snd fields) action_log' vfields Gamma' /\
        List.fold_left (fun vs '(name, v) =>
                          let/opt vs := vs in
                          subst_field_name (struct_fields sig) name v vs
                       ) (combine (map fst fields) (rev vfields)) (Some vs) = Some v) <->
  ...
  Proof.
    ...
    - destr.
      destruct (f Gamma sched_log action_log u) eqn:?; simpl in *; auto.
      + destruct p, p.
        destruct (subst_field_name (struct_fields sig) s v0 vs) eqn:?; simpl in *; auto.
        * rewrite <- IHfields.
  ...
  Lemma arrayinit_ind_ok:
      ...
      elts (iok:
       ...
       Gamma Gamma' action_log action_log' vs v x pos,
        (exists velts,
            interp_list r sigma i
                        Gamma sched_log action_log elts action_log' velts Gamma' /\
            List.fold_left (fun acc v =>
                        let/opt2 pos, vs := acc in
                           ...
                           ) (rev velts) (Some (pos, vs)) = Some (x, v)) <->
                   List.fold_left (fun acc a =>
                          ...
                          ) elts (Some (pos, action_log, vs, Gamma)) = Some (x, action_log', v, Gamma').
  Proof.
    induction elts; simpl; intros; eauto.
    - split; intros A. destruct A; intuition subst. inv H0. simpl in H1. inv H1. auto.
      inv A. eexists; split. constructor. auto.
    - destruct (f Gamma sched_log action_log a) eqn:?; simpl in *; auto.
      + destruct p, p.
        ...
        * rewrite fold_left_none; auto.
          ...
          apply iok in H6. rewrite H6 in Heqo; inv Heqo.
          rewrite rev_app_distr in H1. simpl in H1. rewrite Heqo0 in H1.
          simpl in H1. rewrite fold_left_none in H1; simpl in *; intros; try destr. easy.
          auto. auto.
      + rewrite fold_left_none; auto.
        split; intros A. 2: inv A.
        destruct A; intuition. inv H0.
  ...
  | interp_action_var:
    forall {reg_t ext_fn_t: Type}
      ...
      interp_action r sigma Gamma sched_log action_log (UVar var) action_log v Gamma
  | interp_action_const:
    forall {reg_t ext_fn_t: Type}
           {REnv: Env reg_t}
           (r: REnv.(env_t) (fun _ => val))
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           tau (cst: type_denote tau),
      interp_action r sigma Gamma sched_log action_log (UConst cst) action_log (val_of_value cst) Gamma
  | interp_action_assign:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           k a action_log' Gamma' v,
      interp_action r sigma Gamma sched_log action_log a action_log' v Gamma' ->
      interp_action r sigma Gamma sched_log action_log (UAssign k a) action_log' (Bits 0 [])
  ...
  | interp_action_seq:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           a1 a2 v1 v2 action_log' Gamma' action_log'' Gamma'',
      interp_action r sigma Gamma sched_log action_log a1 action_log' v1 Gamma' ->
      interp_action r sigma Gamma' sched_log action_log' a2 action_log'' v2 Gamma'' ->
      interp_action r sigma Gamma sched_log action_log (USeq a1 a2) action_log'' v2
                    Gamma''
  | interp_action_bind:
    forall {reg_t ext_fn_t: Type}
           {REnv: Env reg_t}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           k a1 a2 v1 v2 action_log' Gamma' action_log'' Gamma'',
      interp_action r sigma Gamma sched_log action_log a1 action_log' v1 Gamma' ->
      interp_action r sigma ((k,v1)::Gamma') sched_log action_log' a2 action_log'' v2 Gamma'' ->
      interp_action r sigma Gamma sched_log action_log (UBind k a1 a2) action_log'' v2
                    (tl Gamma'')
  | interp_action_if:
    forall {reg_t ext_fn_t: Type}
           {REnv: Env reg_t}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           cond b athen aelse v2 action_log' Gamma' action_log'' Gamma'',
      interp_action r sigma Gamma sched_log action_log cond action_log' (Bits 1 [b]) Gamma' ->
      interp_action r sigma Gamma' sched_log action_log' (if b then athen else aelse)
                    action_log'' v2 Gamma'' ->
      interp_action r sigma Gamma sched_log action_log (UIf cond athen aelse) action_log'' v2
                    Gamma''
  | interp_action_read:
    forall {reg_t ext_fn_t: Type}
           {REnv: Env reg_t}
  ...
  | interp_action_write:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           prt idx a v action_log' Gamma',
      interp_action r sigma Gamma sched_log action_log a action_log' v Gamma' ->
      may_write sched_log action_log' prt idx = true ->
      interp_action r sigma Gamma sched_log action_log
                    (UWrite prt idx a)
                    (log_cons idx (LE Logs.LogWrite prt v) action_log')
                    (Bits 0 [])
                    Gamma'
  ...
  | interp_action_intcall:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           f args lv v action_log' Gamma' action_log'' Gamma'',
      interp_list r sigma (interp_action r sigma)
                  Gamma sched_log action_log args action_log' lv Gamma' ->
  ...
  | interp_action_pos:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           p a v Gamma' action_log',
      interp_action r sigma Gamma sched_log action_log a action_log' v Gamma' ->
      interp_action r sigma Gamma sched_log action_log (UAPos p a) action_log' v Gamma'
  ...
  | interp_action_skip:
    forall {reg_t ext_fn_t: Type}
           ...
           (action_log: Log REnv)
           ,
      interp_action r sigma Gamma sched_log action_log (USugar USkip) action_log (Bits 0 []) Gamma
  ...
  | interp_action_constbits:
    forall {reg_t ext_fn_t: Type}
      ...
      interp_action r sigma Gamma sched_log action_log (USugar (UConstBits v))
                    action_log (Bits (List.length l) l) Gamma
  ...
  | interp_action_conststring:
    forall {reg_t ext_fn_t: Type}
      ...
      interp_action r sigma Gamma sched_log action_log (USugar (UConstString s))
                    action_log (Array {| array_type := bits_t 8; array_len := String.length s |}
                                      (List.map (fun x => Bits 8 (vect_to_list x)) (vect_to_list (SyntaxMacros.array_of_bytes s)))) Gamma
  ...
  | interp_action_constenum:
    forall {reg_t ext_fn_t: Type}
      ...
      interp_action r sigma Gamma sched_log action_log (USugar (UConstEnum sig name))
                    action_log (val_of_value (tau:= enum_t sig) (vect_nth sig.(enum_bitpatterns) idx)) Gamma
  ...
  | interp_action_progn:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           aa Gamma' action_log' lv,
      interp_list r sigma (interp_action r sigma) Gamma sched_log action_log aa action_log' lv Gamma' ->
  ...
  | interp_action_let:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           bindings body action_log' action_log'' Gamma' Gamma'' v,
      interp_list_ctx r sigma (interp_action r sigma)
                      Gamma sched_log action_log bindings action_log' Gamma' ->
      interp_action r sigma Gamma' sched_log action_log' body action_log'' v Gamma'' ->
      interp_action r sigma Gamma sched_log action_log (USugar (ULet bindings body))
                    action_log''
                    v
                    (Nat.iter (List.length bindings) (@tl _) Gamma'')
  ...
  | interp_action_when:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           cond athen v2 action_log' Gamma' action_log'' Gamma'',
      interp_action r sigma Gamma sched_log action_log cond action_log' (Bits 1 [true]) Gamma' ->
      interp_action r sigma Gamma' sched_log action_log' athen
                    action_log'' v2 Gamma'' ->
      interp_action r sigma Gamma sched_log action_log (USugar (UWhen cond athen)) action_log'' v2
                    Gamma''
  ...
  | interp_action_switch_nil:
      forall {reg_t ext_fn_t: Type}
             ...
             (sched_log: Log REnv)
             (action_log: Log REnv)
             var default v action_log' Gamma',
        interp_action r sigma Gamma sched_log action_log default action_log' v Gamma' ->
        interp_action r sigma Gamma sched_log action_log (USugar (USwitch var default [])) action_log' v Gamma'
  ...
  | interp_action_switch_ok:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           var default cond body branches
           action_log' Gamma'
           action_log'' Gamma''
           action_log''' Gamma'''
           v0 v v2,
      interp_action r sigma Gamma sched_log action_log var action_log' v0 Gamma' ->
      interp_action r sigma Gamma' sched_log action_log' cond action_log'' v Gamma'' ->
      v = v0 ->
      interp_action r sigma Gamma'' sched_log action_log'' body action_log''' v2 Gamma''' ->
      interp_action r sigma Gamma sched_log action_log
  ...
  | interp_action_switch_ko:
    forall {reg_t ext_fn_t: Type}
      ...
      v <> v0 ->
      interp_action r sigma Gamma'' sched_log action_log''
                    (USugar (USwitch var default branches)) action_log''' v2
                    Gamma''' ->
      interp_action r sigma Gamma sched_log action_log
                    (USugar (USwitch var default ((cond,body)::branches))) action_log''' v2
                    Gamma'''
  ...
  | interp_action_structinit:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           sig fields vfields action_log' Gamma' vs v,
      interp_list r sigma (interp_action r sigma)
                  Gamma sched_log action_log (map snd fields) action_log' vfields Gamma' ->
  ...
  | interp_action_arrayinit:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           tau elements velts action_log' Gamma' vs v,
      interp_list r sigma (interp_action r sigma)
                  Gamma sched_log action_log elements action_log' velts Gamma' ->
        let zeroes := repeat (repeat false (type_sz tau)) (List.length elements) in
        uvalue_of_list_bits (tau:=tau) zeroes = Some vs ->
  ...
  | interp_action_callmodule:
    forall {reg_t ext_fn_t: Type}
           ...
           (sched_log: Log REnv)
           (action_log: Log REnv)
           module_reg_t module_ext_fn_t
           `{finite: FiniteType module_reg_t}
           (fR: module_reg_t -> reg_t)
           (fSigma : module_ext_fn_t -> ext_fn_t)
           fn args lv v action_log' Gamma' action_log'' Gamma'',
      interp_list r sigma (interp_action r sigma)
                  Gamma sched_log action_log args action_log' lv Gamma' ->
      let REnv' := @ContextEnv _ _ in
      interp_action (create REnv' (fun idx => getenv REnv r (fR idx)))
      ...
      interp_action r sigma Gamma sched_log action_log
                    ...
                    (fLog' fR REnv REnv' action_log'' action_log')
                    v
                    Gamma'
  ...
  Lemma ind_ok:
    forall {reg_t ext_fn_t: Type}
        ...
        UntypedSemantics.interp_action r sigma Gamma sched_log action_log a = Some (action_log', v, Gamma') <->
        interp_action r sigma Gamma sched_log action_log a action_log' v Gamma'.
  Proof.
    ...
    - destruct s.
      ...
      + assert (
            forall aa v0 action_log Gamma,
              fold_left
                (fun (acc : option (Log REnv * val * list (var_t * val)))
                     (a : uaction pos_t var_t fn_name_t reg_t' ext_fn_t') =>
                   let/opt3 action_log, _, Gamma := acc
                   in UntypedSemantics.interp_action r sigma Gamma sched_log action_log a) aa
                (Some (action_log, v0, Gamma)) =
              let/opt3 action_log, l, Gamma := fold_left
                                                 (fun (acc : option (Log REnv * list val * list (var_t * val)))
                                                      (a : uaction pos_t var_t fn_name_t reg_t' ext_fn_t') =>
                                                    let/opt3 action_log, l, Gamma := acc in
                                                    let/opt3 action_log, v, Gamma := UntypedSemantics.interp_action r sigma Gamma sched_log action_log a in
                                                    Some(action_log, v::l, Gamma)) aa
                                                 (Some (action_log, [], Gamma)) in
          Some(action_log, hd v0 l, Gamma)).
        {
          clear.
          induction aa; simpl; intros; eauto.
          destruct (UntypedSemantics.interp_action r sigma Gamma sched_log action_log a) eqn:?.
          - destruct p, p; simpl.
            rewrite IHaa.
        ...
        * unfold opt_bind in A; destr_in A; simpl in *; try congruence.
          ...
          econstructor. eapply interp_list_ok; eauto. 2: setoid_rewrite Heqo; auto.
      ...
      + split; intros A.
        * unfold opt_bind in A; repeat destr_in A; inv A.
          ...
          -- inv Heqo.
          -- destruct a.
             destruct (match UntypedSemantics.interp_action r sigma Gamma sched_log action_log var with
           | Some (action_log, v0, Gamma) =>
               match UntypedSemantics.interp_action r sigma Gamma sched_log action_log u with
               | Some (action_log0, v, Gamma0) =>
                   if val_eq_dec v v0
                   then
                    match UntypedSemantics.interp_action r sigma Gamma0 sched_log action_log0 u0 with
                    | Some (action_log1, v1, Gamma1) => Some (action_log1, Some v1, Gamma1)
                    | None => None
                    end
           ...
           | None => None
             ...
             2: rewrite fold_left_none in Heqo. 2: inv Heqo. 2: intros; destr; auto.
          ...
          --
            ...
            ++ destruct a.
               ...
               eapply IHua in H0. 2: simpl; lia.
               eapply interp_action_switch_ko; eauto.
               eapply IHbranches; eauto. simpl; intros; eapply IHua. simpl. lia.
        ...
        * dependent induction A; subst.
          ...
          -- eapply IHua in A1. 2: simpl; lia.
             eapply IHua in A2. 2: simpl; lia.
             eapply IHua in A3. 2: simpl; lia.
             simpl.
             rewrite A1. simpl.
             rewrite A2. simpl.
             rewrite A3. simpl.
             destruct (val_eq_dec v0 v0); try congruence.
             replace (fold_left _ _ _) with (Some (action_log''', Some v2, Gamma''')).
             2: {
               clear.
      ...
      + split; intros A.
        ...
        * dependent induction A. subst.
          ...
          edestruct (structinit_ind_ok r sigma (interp_action r sigma)) as [SIO _].
      ...
      + split; intros A.
        ...
        * dependent induction A. subst.
          unfold zeroes in *. rewrite H0. simpl.
          edestruct (arrayinit_ind_ok r sigma (interp_action r sigma)) as [SIO _].
          intros; eapply IHua. 2: rewrite SIO.
          {
            clear - H2. revert H2.
            induction elements; simpl; intros; eauto. easy.
  ...
  Section Scheduler.
    ...
    Context (rules: rule_name_t -> uaction pos_t var_t fn_name_t reg_t ext_fn_t).
    ...
    | interp_scheduler'_done:
        forall l,
          interp_scheduler' r sigma l Done l
    ...
    | interp_scheduler_intro:
        forall s l,
          interp_scheduler' r sigma log_empty s l ->
