...
namespace prims
{
  ...
  struct array : public std::array<T, len>
  { // Inherit to be able to overload ‘==’
  ...
  struct bits
  {
    ...
    bits<sz> &operator>>=(size_t shift);
    template <bitwidth shift_sz>
    bits<sz> &operator<<=(bits<shift_sz> shift);
    template <bitwidth shift_sz>
    bits<sz> &operator>>=(bits<shift_sz> shift);
  ...
  struct bits<0>
  {
    ...
    bits<0> &operator>>=(size_t) { return *this; }
    template <bitwidth shift_sz>
    bits<0> &operator<<=(bits<shift_sz>) { return *this; }
    template <bitwidth shift_sz>
    bits<0> &operator>>=(bits<shift_sz>) { return *this; }
    ...
    template <typename T>
    static constexpr bits<0> mk(T /*arg*/) { return {}; }
  };
  ...
  namespace literals
  {
    ...
    {
      constexpr auto v = literal_parsing::parse_bin<false, cs...>::v;
      return v;
    }
    ...
    {
      constexpr auto v = literal_parsing::parse_dec<false, cs...>::v;
      return v;
    }
    ...
    {
      constexpr auto v = literal_parsing::parse_hex<false, cs...>::v;
      return v;
    }

    // Immediate binary representation: 4'0010_bv
    template <char... cs>
    constexpr auto operator"" _bv()
    {
      constexpr auto v = literal_parsing::parse_bin<true, cs...>::v;
      return v;
    }

    // Immediate decimal representation: 11'1234_dv
    template <char... cs>
    constexpr auto operator"" _dv()
    {
      constexpr auto v = literal_parsing::parse_dec<true, cs...>::v;
      return v;
    }

    // Immediate hex representation: 0x16'abcd_xv
    template <char... cs>
    constexpr auto operator"" _xv()
    {
      constexpr auto v = literal_parsing::parse_hex<true, cs...>::v;
      return v;
    }
  } // namespace literals
  ...
  template <bitwidth sz>
  struct type_info<bits<sz>>
  {
    static constexpr bitwidth size = sz;
  };

  template <typename T, size_t len>
  struct type_info<array<T, len>>
  {
  ...
  struct _unpack;
  template <typename T>
  static bits<type_info<T>::size> pack(const T val);
  ...
  struct _unpack<array<T, len>, packed_sz>
  {
    ...
    static array<T, len> unpack(bits<packed_sz> bs)
    { // not const&
      array<T, len> unpacked{};
      for (size_t pos = 0; pos < len; pos++)
      { // FIXME check the order of elements
                                  ...
                                  const fmtstyle style, const prefixes prefix)
    {
      ...
      case fmtstyle::full:
        if (sz <= 64)
        {
          ...
          os << " (";
          bits_fmt(os, val, fmtstyle::hex, prefixes::plain);
          os << ", ";
          bits_fmt(os, val, fmtstyle::dec, prefixes::plain);
          os << ")";
        }
        else
        {
...
#define PP_FAIL(rule_name)                             \
  {                                                    \
    std::cout << "Failed " << #rule_name << std::endl; \
  }

#define PP_COMMIT(rule_name)                               \
  {                                                        \
    std::cout << "Committing " << #rule_name << std::endl; \
  }
#else
#define PP_FAIL(rule_name)
#define PP_COMMIT(rule_name)
...
#define PP_FAIL_2(rule_name) PP_FAIL(rule_name)
...
#define DEF_RESET(rl)        \
  RULE_DECL(void, reset, rl) \
  PP_FAIL(rl)

#define DEF_COMMIT(rl)        \
  RULE_DECL(void, commit, rl) \
  PP_COMMIT(rl)
...
#define FAIL()                         \
  {                                    \
    PASTE_EXPANDED_2(reset, RULE_NAME) \
    ();                                \
    return false;                      \
  }
...
#define COMMIT()                        \
  {                                     \
    PASTE_EXPANDED_2(commit, RULE_NAME) \
    ();                                 \
    return true;                        \
  }
...
#define PUSH_DOL(reg)                \
  dlog.push({                        \
      offsetof(struct state_t, reg), \
      sizeof(state_t::reg),          \
      offsetof(struct rwset_t, reg), \
      sizeof(rwset_t::reg),          \
  })
