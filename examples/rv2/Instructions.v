...
Module DecidableInstruction <: DecidableType.
  Definition t := instruction.
  Definition eq := @eq instruction.
  Instance eq_equiv : @Equivalence instruction eq := eq_equivalence.
  Definition eq_dec := instruction_eq_dec.
End DecidableInstruction.

Module InstructionsSet := MSetWeakList.Make DecidableInstruction.
...
Definition example_ISA : ISA := {|
  ...
  ISA_activated_extensions := ExtensionsSet.empty;
|}.

Compute (ISA_instructions_set example_ISA).
