...
Definition classify_instruction_field (f : instruction_field) :=
  match f with
  | opcode => identification_field | rd     => data_field
  | rs1    => data_field           | rs2    => data_field
  | rs3    => data_field           | funct2 => identification_field
  | funct3 => identification_field | funct7 => identification_field
  | immI   => data_field           | immS   => data_field
  | immB   => data_field           | immU   => data_field
  | immJ   => data_field
  end.
...
| funct3_LW     | funct3_LBU     | funct3_LHU      | funct3_SLTI
| funct3_SLTIU  | funct3_ADDI    | funct3_XORI     | funct3_ORI
| funct3_ANDI   | funct3_SLLI    | funct3_SRLI     | funct3_SRAI
| funct3_FENCE  | funct3_SB      | funct3_SH       | funct3_SW
| funct3_BEQ    | funct3_BNE     | funct3_BLT      | funct3_BGE
| funct3_BLTU   | funct3_BGEU    | funct3_PRIV     | funct3_ADDW
| funct3_SUBW   | funct3_SLLW    | funct3_SRLW     | funct3_SRAW
| funct3_LWU    | funct3_LD      | funct3_ADDIW    | funct3_SLLIW
| funct3_SRLIW  | funct3_SRAIW   | funct3_SD       | funct3_FENCE_I
| funct3_CSRRW  | funct3_CSRRS   | funct3_CSRRC    | funct3_CSRRWI
| funct3_CSRRSI | funct3_CSRRCI  | funct3_MUL      | funct3_MULH
| funct3_MULHSU | funct3_MULHU   | funct3_DIV      | funct3_DIVU
| funct3_REM    | funct3_REMU    | funct3_MULW     | funct3_DIVW
| funct3_DIVUW  | funct3_REMW    | funct3_REMUW    | funct3_AMOW
| funct3_AMOD   | funct3_FSGNJ_S | funct3_FSGNJN_S | funct3_FSGNJX_S
| funct3_FMIN_S | funct3_FMAX_S  | funct3_FMV_X_W  | funct3_FEQ_S
| funct3_FLT_S  | funct3_FLE_S   | funct3_FCLASS_S | funct3_FLW
| funct3_FSW.
...
| funct7_SLLW    | funct7_SRLW     | funct7_SRAW     | funct7_SLLIW
| funct7_SRLIW   | funct7_SRAIW    | funct7_MUL      | funct7_MULH
| funct7_MULHSU  | funct7_MULHU    | funct7_DIV      | funct7_DIVU
| funct7_REM     | funct7_REMU     | funct7_MULW     | funct7_DIVW
| funct7_DIVUW   | funct7_REMW     | funct7_REMUW    | funct7_FADD_S
| funct7_FSUB_S  | funct7_FMUL_S   | funct7_FDIV_S   | funct7_FSQRT_S
| funct7_FSGNJ_S | funct7_FMIN_S   | funct7_FCVT_W_S | funct7_FMV_X_W
| funct7_FEQ_S   | funct7_FCLASS_S | funct7_FCVT_S_W | funct7_FMV_W_X
| funct7_SFENCE_VMA.
...
Definition get_instruction_type (i : instruction) :=
  ...
  | RV32A_instruction x =>
    match x with
    | LR_W_32A      => RType | SC_W_32A      => RType | AMOSWAP_W_32A => RType
    | AMOADD_W_32A  => RType | AMOXOR_W_32A  => RType | AMOAND_W_32A  => RType
    | AMOOR_W_32A   => RType | AMOMIN_W_32A  => RType | AMOMAX_W_32A  => RType
    | AMOMINU_W_32A => RType | AMOMAXU_W_32A => RType
    end
  | RV64A_instruction x =>
    match x with
    | LR_W_64A      => RType | SC_W_64A      => RType | AMOSWAP_W_64A => RType
    | AMOADD_W_64A  => RType | AMOXOR_W_64A  => RType | AMOAND_W_64A  => RType
    | AMOOR_W_64A   => RType | AMOMIN_W_64A  => RType | AMOMAX_W_64A  => RType
    | AMOMINU_W_64A => RType | AMOMAXU_W_64A => RType | LR_D_64A      => RType
    | SC_D_64A      => RType | AMOSWAP_D_64A => RType | AMOADD_D_64A  => RType
    | AMOXOR_D_64A  => RType | AMOAND_D_64A  => RType | AMOOR_D_64A   => RType
    | AMOMIN_D_64A  => RType | AMOMAX_D_64A  => RType | AMOMINU_D_64A => RType
    | AMOMAXU_D_64A => RType
    end
  | RV32F_instruction x =>
    match x with
